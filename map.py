...
def append_df_to_gsheet_2(sheet_name, worksheet_title, df, start_date, end_date):
    scope = [
        'https://www.googleapis.com/auth/spreadsheets',
        'https://www.googleapis.com/auth/drive'
    ]
    ...
    try:
        ...
        if len(date_range) != len(df):
            st.error("The number of dates in the range does not match the number of rows in the DataFrame.")
            return

        # Convert the new DataFrame rows to a list of lists and prepend each row with the corresponding date
        new_rows = [[date.strftime("%Y-%m-%d")] + row.tolist() for date, row in zip(date_range, df.values)]

        # Determine the range to update based on the DataFrame's shape
        num_rows, num_cols = df.shape
        start_cell = f'A{last_row}'
        end_col = chr(ord('A') + num_cols)  # Add 1 for the date column
        end_cell = f'{end_col}{last_row + num_rows - 1}'

        # Update the worksheet starting from the next empty row after the last row with data
        worksheet.update(f'{start_cell}:{end_cell}', new_rows)
    except gspread.exceptions.SpreadsheetNotFound:
        st.error(f"Spreadsheet '{sheet_name}' not found.")
    except gspread.exceptions.WorksheetNotFound:
